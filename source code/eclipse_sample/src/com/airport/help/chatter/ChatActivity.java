...
public class ChatActivity extends Activity {
	...
	private Button button_locationRequest;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		button_locationRequest = (Button) findViewById(R.id.button_location);
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_UP:
					if (mListviewHistory.getFirstVisiblePosition() == 0) {
						...
						// "sorry top already", Toast.LENGTH_SHORT).show();
						DbSaveOldMsg.onInit(ChatActivity.this);

						int myId = ConnectedApp.getInstance().getUserInfo()
								.getId();
						int id = ChatService.getInstance().getId();
						ArrayList<ChatEntity> mapFriendsEntity = (ArrayList<ChatEntity>) ChatServiceData
								.getInstance().getCurMsg(2, id);
						ArrayList<Boolean> mapFriendsSelf = (ArrayList<Boolean>) ChatServiceData
								.getInstance().getCurIsSelf(2, id);

						int incre = DbSaveOldMsg.getInstance().getMsg(
								mapFriendsEntity, mapFriendsSelf, myId, id);

						int selection = mListviewHistory
		...
		button_locationRequest.setOnClickListener(new View.OnClickListener() {

			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub

				String st0 = "已发送请求，等待对方接受";
