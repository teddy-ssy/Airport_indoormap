...
public class FriendSearchResultActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String[] strArr0 = searchedResult
				.split(GlobalStrings.searchListDivider);
		...
		for (int p = 1; p <= num; p++) {
			...
			if (uux.getId() != mySelf.getId()) {
				mListU.add(uux);
			}
		}
		mListviewOfResults.setAdapter(new FriendSearchResultAdapter(this,
				mListU));
		...
		mListviewOfResults
				.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						UserInfo mySelf = ConnectedApp.getInstance()
								.getUserInfo();
						...
						if (mySelf.getId() == requestee.getId()) {
							alertDialogBuilder
									.setMessage(
											"sorry, you cannot send yourself friend request!")
									.setCancelable(true)
									.setPositiveButton(
											"Ok",
						...
						} else if (!FriendListInfo.getFriendListInfo()
								.isIdFriend(requestee.getId())) {
							alertDialogBuilder
									.setMessage(
									...
									.setPositiveButton(
											...
											new DialogInterface.OnClickListener() {
												public void onClick(
														DialogInterface dialog,
														int id) {
													UserInfo myUser = ConnectedApp
															.getInstance()
															.getUserInfo();
													FrdRequestEntity reqEnt0 = new FrdRequestEntity(
															myUser, requestee);
													reqEnt0.accept();
													NetworkService
															.getInstance()
															.sendUpload(
																	GlobalMsgTypes.msgFriendshipRequest,
																	reqEnt0.toString());
												}
											})
									.setNegativeButton(
